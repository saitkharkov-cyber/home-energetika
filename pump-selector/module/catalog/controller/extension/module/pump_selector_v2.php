<?php
class ControllerExtensionModulePumpSelectorV2 extends Controller {
    public function index() {
        $this->load->model('extension/module/pump_selector');
        $this->load->model('extension/module/pump_selector_v2');

        $this->document->setTitle('Подбор скважинного насоса — Ranking v2');

        $data = array();
        $data['heading_title'] = 'SMART - подбор скважинного насоса (Ranking v2)';
        $data['action'] = $this->url->link('extension/module/pump_selector_v2', '', true);
        $data['errors'] = array();
        ...
        if (isset($this->request->server['REQUEST_METHOD']) && $this->request->server['REQUEST_METHOD'] == 'POST') {
            if (!empty($this->request->post['website'])) {
                $this->response->redirect($this->url->link('extension/module/pump_selector_v2'));
                return;
            }
            ...
            if (!$errors) {
                ...
                $products = $this->model_extension_module_pump_selector_v2->getRecommendedProducts($requirements);
        ...
        $data['breadcrumbs'][] = array(
            'text' => 'Подбор скважинного насоса — Ranking v2',
            'href' => $this->url->link('extension/module/pump_selector_v2')
        );
