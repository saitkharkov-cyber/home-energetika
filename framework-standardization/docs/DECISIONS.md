...
Для всего проекта `framework-standardization` утверждён единый human-readable формат вывода даты и времени.

### Полная дата и время

Формат:

`DD-MM-YYYY HH:mm:ss Europe/Kyiv`

Пример:

`09-07-2026 21:35:42 Europe/Kyiv`
...
Во всех новых specs, runtime checks, handoff, reports, console output и markdown-документах дата и время должны выводиться в утверждённом human-readable формате.

Часовой пояс указывается явно как `Europe/Kyiv`, если выводится дата вместе со временем.

Для machine-readable output допускается отдельное поле с Unix timestamp, ISO 8601 или RFC 3339 datetime, если это нужно для интеграции, сортировки или автоматической обработки.

Human-readable вывод по умолчанию должен сохранять основной формат:

`DD-MM-YYYY HH:mm:ss Europe/Kyiv`

### Запрещено по умолчанию

Не использовать в новых документах и отчётах как основной human-readable формат:

* `MM/DD/YYYY`;
* локализованные строки без timezone;
* относительные даты вроде `сегодня`, `вчера`, `завтра`;
* время без указания timezone, если оно важно для runtime/check/report.
...
## 10-07-2026 - Voltage normalization policy approved

### Решение

Для характеристики `Напряжение` утверждена normalization policy:
...
* canonical unit: `В`;
* normalized value type: `integer_enum`;
* allowed canonical values: string values `'220'`, `'380'`.
...
* `220`;
* formatting-only forms `220V`, `220 В`;
* nominal `230` forms;
* one-phase `230` forms;
* ranges `200-240`, `210-240`, `220-230`, `220-240`.

Class `380`:

* `380`;
* formatting-only forms `380V`, `380 В`;
* nominal `400` forms;
* three-phase `400` forms;
* ranges `380-400`, `380-420`.

Phase may be used as additional class signal. Phase contradiction is never silently normalized and must return `review_required`, `canonical_value: null`, warning `phase_voltage_class_conflict`, and `ambiguity_reason: phase_voltage_class_conflict`.

Frequency `50 Гц` / `60 Гц` is not part of canonical value and must remain diagnostics/metadata only.

### Mixed and outside-policy behavior

If a value contains both classes `220` and `380`, result must be:

```text
status: review_required
canonical_value: null
warnings contains mixed_voltage_classes
ambiguity_reason: mixed_voltage_classes
```

If a value contains outside-policy voltage, result must be:

```text
status: review_required
canonical_value: null
warnings contains voltage_outside_allowed_classes
ambiguity_reason: voltage_outside_allowed_classes
```

### Unchanged semantics

`unchanged` is allowed only when all conditions are true:

* source attribute is canonical `15`;
* raw value after trim is exactly `220` or `380`;
* normalized canonical value equals raw value.

All alias rows and all formatting/nominal/range transformations are `normalized`, not `unchanged`.
...
## 2026-07-14 — срочная ручная стандартизация отделена от развития фреймворка

### Решение

Развитие универсального фреймворка стандартизации временно приостановлено.

Текущий главный приоритет проекта — привести в порядок характеристики товаров категории `11900213` «Скважинные насосы» с учётом вложенных подкатегорий и выдать фактический результат.

Для этого утверждён отдельный ускоренный путь:

1. ChatGPT подготавливает пошаговые SQL-запросы.
2. Пользователь самостоятельно выполняет их в phpMyAdmin на продуктивной базе.
3. После каждого запроса пользователь передаёт фактический результат.
4. Следующий запрос готовится только после проверки предыдущего результата.
5. Развитие кода фреймворка не является обязательным условием текущей стандартизации.

### Использование существующих наработок

Срочный путь использует уже полученные результаты проекта:

* карту дублирующихся характеристик;
* утверждённые канонические атрибуты;
* предметные решения из `LEGACY_DECISIONS.md`;
* результаты аудита локальной копии базы;
* разработанные правила нормализации;
* проверенный порядок переноса, проверки и очистки;
* результаты обработки контрольной группы Sumoto.

Повторное исследование категории с нуля не выполняется. Состояние продуктивной базы только сверяется с уже собранной картой.

### Граница с фреймворком

Ручная работа через phpMyAdmin:

* не является производственным применением фреймворка;
* не означает готовность SQL/apply-слоя фреймворка;
* не изменяет статусы завершённости его ступеней;
* не требует реализации общего интерфейса нормализаторов;
* не требует пакетного исполнителя;
* не требует веб-интерфейса.

После выдачи результата развитие фреймворка может быть продолжено с текущей зафиксированной точки.

### Рабочая схема

Для каждой характеристики применяется следующий порядок:

1. Контрольный `SELECT`.
2. Определение точного списка безопасных изменений.
3. Фиксация количества строк и уникальных товаров.
4. Резервная выборка затрагиваемых строк.
5. Отдельный изменяющий SQL-запрос.
6. Контрольная проверка результата.
7. Отдельная очистка только подтверждённых старых строк.
8. Итоговая проверка.
9. Фиксация оставшихся неоднозначных случаев.

Неоднозначные значения не изменяются предположительно и не блокируют обработку безопасных строк.

### Производственная граница

Локальный проект, Codex и фреймворк не подключаются к продуктивной базе.

Продуктивные реквизиты не сохраняются в репозитории или локальной конфигурации проекта.

Все запросы на продуктивной базе выполняет только пользователь непосредственно через phpMyAdmin.

Разовые SQL-запросы, экспорты строк и резервные данные не добавляются в Git.

### Git и рабочая ветка

Работа продолжается в ветке `main`.

Отдельная ветка и отдельная локальная копия репозитория не создаются.

В Git фиксируются только:

* утверждённый план;
* постоянные решения;
* подтверждённые итоговые результаты;
* список оставшихся исключений без секретов и необработанных дампов.

### Ссылка

Оперативный порядок работ определён в:

`docs/PRODUCTION_STANDARDIZATION_EXECUTION.md`
