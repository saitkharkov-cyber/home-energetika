...
После создания в `PROJECT_MASTER_SUMMARY.md` добавить:
...
А следующий логичный шаг заменить на:
...
Документ должен зафиксировать, как Framework формирует SQL preview, какие блокировки безопасности учитывает и почему SQL не выполняется автоматически.
