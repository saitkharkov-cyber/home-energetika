...
`RUNTIME_CHECKS.md` обновляется только после реального DB-readonly запуска, ручной проверки на данных или существенного изменения runtime-поведения. Результаты обычных syntax checks, unit/static tests и стандартных регрессионных тестов сами по себе в документ не добавляются, если они не выявили новый runtime-факт или проблему.
