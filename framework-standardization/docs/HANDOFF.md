...
Последний закрытый шаг: `DB readonly canonical resolver`.

Последний коммит: `1737900 Add DB readonly canonical resolver`.
...
Локальный dump/config и первый DB-backed resolver готовы. Следующий шаг - отдельное архитектурное решение, как безопасно подключать DB-backed resolver к composition layer / factory для конкретного job/scope, не ломая dry-run boundary.
...
```text
HEAD/main/origin/main = 1737900 Add DB readonly canonical resolver
```
