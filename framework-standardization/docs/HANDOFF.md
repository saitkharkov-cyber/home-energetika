...
Последний закрытый шаг: `DB readonly pump diameter job`.

Последний коммит: `abc539f Add DB readonly pump diameter job`.
...
Локальный dump/config, первый DB-backed resolver и отдельный DB-readonly job готовы. Следующий шаг - mini-spec для `DbReadOnlyPipelineFactory` / composition layer, чтобы безопасно подключать DB-backed resolver для конкретного job/scope, не ломая dry-run boundary.
...
```text
HEAD/main/origin/main = abc539f Add DB readonly pump diameter job
```
