...
Последний закрытый шаг: `local DB dump checklist`.

Последний коммит: `3ad3660 Add local DB dump checklist`.
...
Следующий шаг - подготовить локальный OpenCart dump или локальную БД без персональных и операционных данных по `docs/DUMP_LOCAL_DB_CHECKLIST.md`. Live DB не использовать.
...
```text
HEAD/main/origin/main = 3ad3660 Add local DB dump checklist
```
