...
## 2026-07-09  Controlled alias cleanup apply для максимального напора

### Commit/context

- implementation commit: `f02a421 Add max head alias cleanup controlled apply`
- previous preview commit: `771283d Add max head alias cleanup readonly preview`
- alias cleanup gate spec commit: `71bf9b6 Document max head alias cleanup gate`
...
- target table: `oc_product_attribute`

### Commands/classes

- `framework-standardization/bin/db-controlled-alias-cleanup-max-head.php`
- `framework-standardization/src/Apply/DbControlledAliasCleanupMaxHeadCommand.php`

### Safety

- dry-run by default;
- real cleanup only with `--confirm-apply`;
- controlled local dump only;
- hard stop before `buildPlan` for non-controlled runtime;
- DELETE only exact rows from computed plan;
- DELETE identity: `product_id + attribute_id + language_id + text`;
- transaction;
- verification before commit;
- rollback on mismatch/error;
- no production/cache;
- no cache rebuild;
- no `oc_attribute` / `oc_attribute_description` changes;
- canonical `attribute_id = 12` not deleted.

### Apply result

- `planned_delete_count: 81`
- `actual_deleted_count: 81`
...
- repeat dry-run `planned_delete_count: 0`
- `remaining_alias_rows: 14`
- `remaining_not_removable_rows: 14`
- `sql_applied: 0`
- `product_data_changed: 0`
...
- Phase 2 alias cleanup completed on controlled local dump;
- 81 safely removable alias rows removed;
- 14 unresolved/excluded alias rows intentionally kept;
- max head is locally standardized in two phases:
  - Phase 1 canonical value apply;
  - Phase 2 alias cleanup.

### Boundary confirmation

- это doc-only update;
- PHP/code не менялись;
- SQL/apply повторно не выполнялись;
- production/cache не трогались;
- cache rebuild не выполнялся;
- `HANDOFF.md` не менялся;
- `DECISIONS.md` не менялся.

## 2026-07-09  Generic canonical value apply через contract: diagnostic dry-run

### Commit/context

- generic canonical apply diagnostic commit: `a0864c7 Add generic attribute apply dry-run diagnostics`
- previous stable point: `2a28f9f Document PowerShell 7 command chaining`
...
- создан generic controlled canonical apply command;
- команда читает explicit contract;
- runtime allowlist проверяется до `createPdo` / DB SELECT;
- unsupported normalizer blocked;
- `--confirm-apply` сейчас hard-stop с ошибкой `confirm_apply_not_enabled_for_generic_attribute_apply_step_d`;
- canonical rows не используются как source candidate values;
- source/alias rows не меняются;
- UPDATE/INSERT/DELETE path не реализован и не выполнялся.

### Dry-run result на already-cleaned local dump
...
- Step D command создан успешно как diagnostic dry-run;
- это не полноценное доказательство Phase 1 source-based apply, потому что current local dump уже находится после Phase 2 alias cleanup;
- source rows для 481 уже применённых canonical rows удалены, поэтому `source_based_plan_available: 0`;
- `canonical_only_verified_count: 481` показывает, что canonical-состояние присутствует, но это diagnostic count, а не source-based proof;
- `expected_counts_match: 0` и `post_apply_verification_ok: 0` являются ожидаемым честным результатом для already-cleaned state;
- полноценная проверка generic Phase 1 требует отдельного состояния до alias cleanup: pre-cleanup dump или controlled fixture.

### Safety

- `--confirm-apply` не запускался;
- SQL/apply/UPDATE/INSERT/DELETE не выполнялись;
- source alias rows не менялись;
- canonical rows не удалялись;
- production/cache не трогались;
- cache rebuild не выполнялся;
- prototype max-head commands/classes не менялись;
- generic alias cleanup files не менялись;
- existing contract и runtime configs не менялись.

### Next direction

- подготовить pre-alias-cleanup fixture/dump для source-based проверки generic canonical apply;
- либо явно описать fixture requirement перед включением generic confirm apply.
...
## 2026-07-09  Generic alias cleanup через contract для максимального напора

### Commit/context

- contract commit: `b778f3b Add max head attribute contract`
- generic readonly preview commit: `68cafa6 Add generic alias cleanup readonly preview`
- generic controlled dry-run commit: `e6fb74f Add generic alias cleanup controlled dry run`
...
- generic readonly preview читает contract, а не hardcoded max-head fields;
- generic controlled alias cleanup dry-run читает contract;
- runtime allowlist проверяется до `createPdo` / DB SELECT;
- unsupported normalizer blocked;
- contract-gate для `confirmation_required` и `allow_confirm_apply` добавлен в apply class;
- prototype max-head commands/classes не менялись.

### Generic readonly preview result

- `total_alias_rows_in_scope: 14`
- `safely_removable_alias_rows: 0`
- `not_removable_alias_rows: 14`
- `unresolved_or_excluded_value: 14`
- `expected_counts_match: 1`
- `delete_executed: 0`
- `sql_applied: 0`

### Generic controlled dry-run result

- `planned_delete_count: 0`
- `planned_keep_alias_count: 14`
- `remaining_alias_rows: 14`
- `remaining_not_removable_rows: 14`
- `not_removed_unresolved_or_excluded_count: 14`
- `expected_counts_match: 1`
- `post_cleanup_verification_ok: 1`
- `sql_applied: 0`
- `product_data_changed: 0`
...
- `--confirm-apply` не запускался;
- DELETE/apply не выполнялись;
- production/cache не трогались;
- cache rebuild не выполнялся;
- `oc_attribute` / `oc_attribute_description` не трогались.

### Interpretation

- Step B generic readonly alias cleanup preview завершён успешно;
- Step C generic controlled alias cleanup dry-run завершён успешно;
- Phase 2 alias cleanup теперь частично обобщена через contract;
- следующий direction: Step D generic canonical value apply dry-run через contract.

### Boundary confirmation

- это doc-only update;
- PHP/code не менялись;
- SQL/apply/DELETE не выполнялись;
- `--confirm-apply` не запускался;
- production/cache не трогались;
- cache rebuild не выполнялся;
- `HANDOFF.md` не менялся;
...
## 2026-07-09  Generic canonical apply synthetic fixture dry-run

### Commit/context

- fixture dry-run commit: `501eadc Add generic canonical apply synthetic fixture dry run`
- fixture spec commit: `cacf7bf Document generic canonical apply fixture requirement`

### Scope

- `target_key: max_head_synthetic_fixture`
- target meaning: `максимальный напор synthetic fixture`
- `canonical_attribute_id: 12`
- `alias_attribute_ids: 101, 119, 81`
- `normalizer_key: simple_meters`
- fixture-only contract: `framework-standardization/config/attribute-contracts/fixtures/max_head_synthetic_fixture.php`
- fixture rows: `framework-standardization/fixtures/generic-canonical-apply/max_head_synthetic_rows.php`
- CLI: `framework-standardization/bin/fixture-canonical-apply-dry-run.php`
- runner: `framework-standardization/src/Fixture/GenericCanonicalApplyFixtureDryRun.php`

### Что проверено

- source-based plan logic работает без DB;
- fixture покрывает update, insert, already_applied, unresolved, duplicate/conflict, out_of_scope;
- mixed valid+unresolved case не планирует write и считается conflict;
- `source_based_plan_available` учитывает только update/insert, не `already_applied`;
- `dry_run_expected_counts_ok` является dry-run success marker;
- `post_apply_verification_ok` остаётся `0`, потому что apply не выполнялся;
- fixture safety contract валидирует:
  - `safety.fixture_only === true`
  - `safety.db_allowed === false`
  - `safety.confirm_apply_allowed === false`
  - `safety.production_ready === false`
  - `safety.cache_rebuild_allowed === false`
...
Запуск с `--confirm-apply` завершился hard-stop:

```text
fixture_canonical_apply_dry_run_error: fixture_confirm_apply_not_allowed
```

## 2026-07-09  Generic canonical apply write-path structure: implementation-only

### Commit/context

* write-path structure commit: `50fe53b Add generic attribute apply write-path structure`
...
* добавлена implementation-only structure для будущего generic canonical UPDATE/INSERT write-path;
* `--confirm-apply` по-прежнему hard-stop;
* confirm gate вынесен в core class;
* добавлены disabled skeleton methods для будущего write-path;
* SQL UPDATE/INSERT strings не добавлялись;
* transaction/write execution не запускается;
* dry-run behavior не должен менять данные.

### Новые output markers

DB dry-run теперь выводит:
...
Эти markers означают:

* structure для будущего write-path присутствует;
* confirm apply не включён;
* выполнение write-path не включено;
* текущий шаг является implementation-only.

### DB dry-run result на already-cleaned local dump

Ожидаемое diagnostic состояние сохраняется:
...
Запуск generic canonical apply с `--confirm-apply` завершился hard-stop:

`controlled_attribute_apply_error: confirm_apply_not_enabled_for_generic_attribute_apply_implementation_only`

Это ожидаемо. На этом этапе `--confirm-apply` не разрешён и не включает write-path.
...
* SQL/apply/UPDATE/INSERT/DELETE не выполнялись;
* `--confirm-apply` не запускал apply;
* production/cache не трогались;
* cache rebuild не выполнялся;
* source alias rows не менялись;
* canonical rows не удалялись;
* `oc_attribute` / `oc_attribute_description` не менялись;
* fixture files не менялись;
* prototype max-head files не менялись;
* existing contract и runtime configs не менялись;
* docs кроме этого runtime check не менялись.

### Interpretation

* generic canonical apply получил каркас будущего write-path;
* actual generic UPDATE/INSERT write-path всё ещё не включён;
* текущий шаг не является apply readiness;
* текущий шаг не разрешает `--confirm-apply`;
* следующий gate должен отдельно разрешить реализацию controlled transaction UPDATE/INSERT path либо дальнейшую подготовку verification перед включением confirm apply.

### Boundary confirmation

* это implementation-only check;
* SQL/apply не выполнялись;
* `--confirm-apply` не разрешён;
* production/cache не трогались;
* cache rebuild не выполнялся;
* `HANDOFF.md` не менялся;
* `DECISIONS.md` не менялся.
...
## 12-07-2026 — Characteristic registry DB-readonly check для скважинных насосов
...
language_id: 1
runtime_mode: db_readonly
db_source: controlled_local_dump
implementation_commit: 1fee072 Mark registry DB connection after discovery
```

Проверены полный read-only discovery характеристик и построение registry через `DbReadOnlyCharacteristicDiscovery`, `CharacteristicRegistryBuilder` и `DbReadOnlyCharacteristicRegistryOrchestrator`.

### Результат

```text
db_connected_actual: 1
payload_db_connected: 1
db_connected_marker_mismatch: 0

total_discovered: 72
contract_required: 63
contract_draft: 0
contract_approved: 9
normalizer_required: 6
normalizer_ready: 2
read_only_ready: 2
blocked: 1
```

Предыдущий реальный запуск показывал `payload_db_connected: 0`. После commit `1fee072` DB marker соответствует фактическому подключению к тому же controlled local dump.

### Контрольные результаты

```text
matched_legacy_ids: 44, 121, 13, 12, 100, 15, 57, 73, 76
read_only_ready_ids: 15, 57
blocked_ids: 73
unexpected_registry_delta: none
```

`15` и `57`  строки напряжения, готовые к read-only обработке. `73`  утверждённое исключение. Девять approved rows относятся к четырём утверждённым характеристикам. Шесть строк требуют normalizer, ещё 63 строки требуют человеческого решения по контракту.
...
Использован только существующий read-only `SELECT` внутри `DbReadOnlyCharacteristicDiscovery` к controlled local dump. Дополнительные SQL-запросы, DB writes, schema operations, pipeline, raw-values inventory, normalization, SQL generation/apply, изменение product data, production и cache actions не выполнялись. Временные файлы удалены.
