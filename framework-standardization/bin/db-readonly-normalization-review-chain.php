...
if (PHP_SAPI !== 'cli') {
    fwrite(STDERR, "db-readonly-normalization-review-chain.php must be executed from CLI.\n");
    exit(1);
}

try {
    if (!isset($argv[1]) || trim($argv[1]) === '') {
        throw new \InvalidArgumentException('usage: php bin/db-readonly-normalization-review-chain.php path/to/runtime.php --category-id=ID --attribute-ids=1,2 --canonical-attribute-id=ID --canonical-unit=m [--format=plain|markdown]');
    }
    ...
    if ($options['format'] === 'markdown') {
        printReviewChainMarkdown($result, $options['format']);
    } else {
        printReviewChainPlain($result, $options['format']);
    }

    exit(0);
} catch (\Exception $e) {
    fwrite(STDERR, 'normalization_review_chain_error: ' . $e->getMessage() . "\n");
    exit(1);
}
...
function printReviewChainPlain(array $result, $format)
{
    echo "runtime_mode: db_readonly\n";
    echo "command: normalization_review_chain\n";
    ...
    echo 'format: ' . $format . "\n";
    echo 'pending_review_count: ' . $result['pending_review_count'] . "\n";
    echo 'unresolved_count: ' . $result['unresolved_count'] . "\n";
    echo 'skipped_count: ' . $result['skipped_count'] . "\n";
    echo "\n";
    echo "review_chain:\n";

    foreach ($result['review_rows'] as $row) {
    ...
    foreach ($result['unresolved_rows'] as $row) {
        echo '- review_id: ' . $row['review_id'] . "\n";
        echo '  product_id: ' . $row['product_id'] . "\n";
        echo '  attribute_id: ' . $row['attribute_id'] . "\n";
        echo '  attribute_name: ' . valueOrNone($row['attribute_name']) . "\n";
...
function printReviewChainMarkdown(array $result, $format)
{
    echo "# DB-readonly normalization review-chain\n";
    echo "\n";
    echo "- runtime_mode: db_readonly\n";
    echo "- command: normalization_review_chain\n";
...
{
    echo 'review_chain_generated: ' . (int) $result['review_chain_generated'] . "\n";
